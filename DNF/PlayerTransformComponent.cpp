...
void PlayerTransformComponent::Update()
{
	...
	case PlayerStatusComponent::ePlayerState::Run:
		...
		if		(eDirX::Left != _dirX)	{ if (Input::GetButton(VK_LEFT) && false == Input::GetButton(VK_RIGHT)) { SetDirX(eDirX::Left); } }
		else if (eDirX::Right != _dirX) { if (Input::GetButton(VK_RIGHT) && false == Input::GetButton(VK_LEFT)) { SetDirX(eDirX::Right); } }

		if		(eDirY::Up != _dirY)	{ if (Input::GetButton(VK_UP) && false == Input::GetButton(VK_DOWN)) { SetDirY(eDirY::Up); } }
		else if (eDirY::Down != _dirY)	{ if (Input::GetButton(VK_DOWN) && false == Input::GetButton(VK_UP)) { SetDirY(eDirY::Down); } }
		break;
